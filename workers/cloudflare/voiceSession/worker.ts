...
async function handleConversationBackup(
  request: Request,
  env: Env
): Promise<Response> {
  try {
    ...
    const statements = [
      ...
      env.D1_AVATAR_SESSIONS.prepare(
        "UPDATE sessions SET chat_data = chat_data || ? WHERE id = ?"
      ).bind(messageText + "\n", sessionId),
    ];
...
async function handleAudioBackup(
  request: Request,
  env: Env
): Promise<Response> {
  try {
    ...
    const statements = [
      ...
      env.D1_AVATAR_SESSIONS.prepare(
        "UPDATE sessions SET audio_data = audio_data || ? WHERE id = ?"
...
async function handleEndSession(request: Request, env: Env): Promise<Response> {
  try {
    ...
    const statements = [
      ...
      env.D1_AVATAR_SESSIONS.prepare(
        "SELECT audio_data FROM sessions WHERE id = ?"
      ).bind(sessionId),
    ...
    const session = selectResult.results[0] as { audio_data: string };
