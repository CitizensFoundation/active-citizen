  ...
  async renderRefinePrompt(results: IEngineAffectedEntity[]) {
    const messages = [
      new SystemChatMessage(
        `
        You are an AI specialist trained to analyze complex problem statements with a sub problem and refine entities affected by them.
        Entities can range from individuals, groups, systems, to the planet or even inanimate objects.

        Adhere to the following guidelines:

        1. Your task is to refine the entities and the negative and positive effects generated by you previously.
        1. Output all entities impacted by the main problem and its sub problem.
        ...
        9. Expaind on the reasons for the negative and positive effects if needed to make it more clear and focused.
        10. Review the problem statement and sub-problem and provide an output in the following format: [ { name: name, negativeEffects: [ reason ], positiveEffects: [ reason ] } ].
        11. Ensure a methodical, step-by-step approach.
        `
      ),
