...
async function fetchChoices(
  ...
  try {
    const response = await fetch(
      `${PAIRWISE_API_HOST}/questions/${questionId}/choices?utm_source=${
        utmSource || ""
      }`,
      {
        method: "GET",
...
export async function exportChoiceVotes(
  questionId: number,
  utmSource: string,
  outputFolder: string = "tmp"
) {
  console.log(
    `Exporting choice votes for question ${questionId} with utm_source ${utmSource}`
  );
  ...
  for (let choice of choices) {
    const votes = (await fetchVotes(choice.id, utmSource)) as {
      winning_votes: AoiVoteData[];
      losing_votes: AoiVoteData[];
    };
